<?php
  ...
  $despesa = "SELECT SUM(VALOR) AS VALOR, FORMA_PAGAMENTO AS PAGAMENTO FROM DESPESA WHERE ID_USUARIO = $idUsuario AND MONTH(DATA) = $mesAtual AND YEAR(DATA) = $anoAtual AND FORMA_PAGAMENTO <> 2 GROUP BY FORMA_PAGAMENTO;";
  $despesaCard = "SELECT SUM(VALOR) AS VALOR, FORMA_PAGAMENTO AS PAGAMENTO, TIPO FROM DESPESA INNER JOIN CARTAO ON DESPESA.ID_CARTAO = CARTAO.ID_CARTAO WHERE DESPESA.ID_USUARIO = $idUsuario AND MONTH(DATA) = $mesAtual AND YEAR(DATA) = $anoAtual AND FORMA_PAGAMENTO = 2 GROUP BY TIPO LIMIT 12;";
...
function grafFormaPagamento() {
  var teste = <?php echo $teste6; ?>;

  if (teste == 0) {
    document.getElementById("avisoDespPgto").style.display = "none";
